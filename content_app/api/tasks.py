...
def convert_to_hls(video_id):
    ...
    for label, opts in resolutions.items():
        ...
        cmd = [
            ...
            "-vf", f"scale={opts['scale']}:force_original_aspect_ratio=decrease",
            "-c:a", "aac", "-ar", "48000",
            "-c:v", "h264", "-profile:v", "main", "-crf", "20", "-sc_threshold", "0",
            "-g", "48", "-keyint_min", "48",
            "-b:v", opts["bitrate"], "-maxrate", opts["bitrate"], "-bufsize", "4200k",
            "-b:a", "128k", "-hls_time", "4", "-hls_playlist_type", "vod",
...
def create_thumbnail(video_id):
    def args(source, target):
        return [
            "ffmpeg", "-ss", "00:00:05", "-i", str(source),
            "-vframes", "1", "-q:v", "2", str(target)
        ]
...
def create_preview(video_id):
    def args(source, target):
        return [
            "ffmpeg", "-ss", "00:00:02", "-i", str(source),
            "-t", "5", "-c:v", "libx264", "-c:a", "aac", str(target)
        ]
